...
Una ultima cosas que habrás notado: solo definimos un `Filosofo`, y no parecemos hacer nada con el. Rust es un lenguaje ‘basado en expresiones’, lo que significa que casi cualquier cosa en Rust es una expresion que retorna un valor. Esto es cierto para las funciones también, la ultima expresion es retornada automaticamente. Debido a que creamos un nuevo `Filosofo` como la ultima expresion de esta funcion, terminamos retornandolo.
