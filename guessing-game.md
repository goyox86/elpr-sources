...
Despues, hemos agregado otra linea `use`: `use rand::Rng`. En unos momentos estaremos haciendo uso de un metodo, y esto requiere que `Rng` este disponible para que funcione. La idea basica es la siguiente: los metodos en algo llamado ‘traits’ (Rasgos), y para que el metodo funcione neceita que el trait este disponible. Para mayores detalles dirigete a la sección [Rasgos][traits] section
