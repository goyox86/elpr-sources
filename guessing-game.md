...
La primera cosa que hemos hecho es cambiar la primera linea. Ahora dice `extern crate rand`. Debiado a que declaramos `rand` en nuestra seccion `[dependencies]`, podemos usar `extern crate` para hacerle saber a Rust que estaremos haciendo uso de ello. Esto es qeuivalente a un `use rand;`, de manera que podamos hacer uso de lo que sea dentro del crate `rand` a traves del prefijo `rand::`.
